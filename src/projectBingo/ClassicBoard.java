...
public class ClassicBoard extends Board {
    ...
    public int detectWin() {
        ...
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 5; j++) {
                if (matrix[i][j].getCompletionState() == 1) {
                    count1++;
                } else if (matrix[i][j].getCompletionState() == 2) {
                    count2++;
                }
            }
        }

        if (count1 + count2 == 25 && count1 >= 13) {
            return 1;
        } else if (count1 + count2 == 25) {
            return 2;
        }
